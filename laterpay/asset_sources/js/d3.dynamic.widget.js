...
var LPCurve = function(container) {
    ...
    svg.append('rect').attr('class', 'lp_dynamicPricing__background');
    ...
    svg.append('line').attr('class', 'lp_dynamicPricing__default-price');
    svg
    .append('text')
    .attr('text-anchor', 'middle')
    .attr('class', 'lp_dynamicPricing__default-price')
    ...
    .attr('class', 'lp_dynamicPricing__start-price-input')
    .attr('width','44px')
    .attr('height','24px')
    .html('<input type="text">')
    .attr('display','none');
    svg.append('text').attr('class', 'start-price').attr('text-anchor', 'end');
    svg.append('text').attr('class', 'lp_dynamicPricing__currency').attr('text-anchor', 'end').text(this.currency);
    ...
    svg
    .insert('foreignObject')
    .attr('class', 'lp_dynamicPricing__end-price-input')
    .attr('width','44px')
    .attr('height','24px')
    .html('<input type="text">')
    .attr('display','none');
    ...
    jQuery('body').on('click', '.start-price, .lp_dynamicPricing__currency, .start-price-triangle', function() {
        lpc.toggleStartInput('show');
    });
    jQuery('.lp_dynamicPricing__start-price-input input').change(function() {
        lpc.toggleStartInput('hide'); // have to leave one event only: update or focusout. Depends on point of view.
    });
    jQuery('.lp_dynamicPricing__start-price-input input').focusout(function() {
        lpc.toggleStartInput('hide'); // have to leave one event only: update or focusout. Depends on point of view.
    });
    jQuery('.lp_dynamicPricing__start-price-input input').keydown(function(e) {
        // hide input on Enter
    ...
    jQuery('body').on('click', '.end-price, .lp_dynamicPricing__currency, .end-price-triangle', function() {
        lpc.toggleEndInput('show');
    });
    jQuery('.lp_dynamicPricing__end-price-input input').change(function() {
        lpc.toggleEndInput('hide'); // have to leave one event only: update or focusout. Depends on point of view.
    });
    jQuery('.lp_dynamicPricing__end-price-input input').focusout(function() {
        lpc.toggleEndInput('hide'); // have to leave one event only: update or focusout. Depends on point of view.
    });
    jQuery('.lp_dynamicPricing__end-price-input input').keydown(function(e) {
        // hide input on Enter
        var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
        if (key === 13) {
            e.preventDefault();
...
LPCurve.prototype.plot = function() {
    ...
    svg.select('.lp_dynamicPricing__background')
    ...
    // Default price
    svg.select('line.lp_dynamicPricing__default-price')
        .transition().duration(dragging ? 0 : 250)
        .attr({
            x1: 0,
    ...
    svg.select('text.lp_dynamicPricing__default-price')
        .transition().duration(dragging ? 0 : 250)
        .attr({
            x: width / 2,
    ...
    svg.select('text.lp_dynamicPricing__currency')
        .datum((self.data)[0])
        .call(dragYAxisBehavior)
        .transition().duration(dragging ? 0 : 250)
        .attr({
    ...
    // START PRICE INPUT
    svg.select('.lp_dynamicPricing__start-price-input')
        .datum((self.data)[0])
        .call(dragYAxisBehavior)
        .transition().duration(dragging ? 0 : 250)
        .attr({
    ...
    svg.select('rect.end-price')
        ...
        .attr({
            x: function()  {
                    if (
                        jQuery('.lp_dynamicPricing__end-price-input') &&
                        jQuery('.lp_dynamicPricing__end-price-input').is(':visible')
                    ) {
                        return width;
                    } else {
                        return width + 16;
    ...
    // END PRICE INPUT
    svg.select('.lp_dynamicPricing__end-price-input')
        .datum((self.data)[self.data.length - 1])
        .call(dragYAxisBehavior)
        .transition().duration(dragging ? 0 : 250)
        .attr({
            x: function()  {
                if (
                    jQuery('.lp_dynamicPricing__end-price-input') &&
                    jQuery('.lp_dynamicPricing__end-price-input input').is(':visible')
                ) {
                    return width + 2;
                } else {
                    return width + 20;
...
LPCurve.prototype.toggleStartInput = function(action) {
    var dragShake   = 1,
        data        = lpc.get_data(),
        plotPrice   = data[0].y,
        inputPrice = jQuery('.lp_dynamicPricing__start-price-input input').val();
    ...
    if (action === 'hide') {
        if (inputPrice > this.maxPrice) {
            jQuery('.lp_dynamicPricing__start-price-input input').val(this.maxPrice);
            data[0].y = this.maxPrice;
            data[1].y = this.maxPrice;
        } else if (inputPrice < this.minPrice && inputPrice !== 0) {
            jQuery('.lp_dynamicPricing__start-price-input input').val(this.minPrice);
        ...
        jQuery('.lp_dynamicPricing__start-price-input').hide();
        jQuery('path.start-price-triangle, text.lp_dynamicPricing__currency, text.start-price').show();
        lpc.plot();
    } else if (action === 'show') {
        jQuery('rect.start-price').attr('width', '50px');
        jQuery('path.start-price-triangle, text.lp_dynamicPricing__currency, text.start-price').hide();
        jQuery('.lp_dynamicPricing__start-price-input').show();
        jQuery('.lp_dynamicPricing__start-price-input input').val( plotPrice.toFixed(2) );
    } else if (action === 'update') {
        if (jQuery('.lp_dynamicPricing__start-price-input input').is(':visible')) {
...
LPCurve.prototype.toggleEndInput = function(action) {
    var dragShake   = 1,
        data        = lpc.get_data(),
        plotPrice   = data[2].y,
        inputPrice  = jQuery('.lp_dynamicPricing__end-price-input input').val(),
    ...
    if ( action === 'hide' ) {
        if (inputPrice > this.maxPrice) {
            jQuery('.lp_dynamicPricing__end-price-input input').val(this.maxPrice);
            data[0].y = this.maxPrice;
            data[1].y = this.maxPrice;
        } else if (inputPrice < this.minPrice && inputPrice !== 0) {
            jQuery('.lp_dynamicPricing__end-price-input input').val( this.minPrice );
        ...
        jQuery('.lp_dynamicPricing__end-price-input').hide();
        jQuery('path.end-price-triangle, text.lp_dynamicPricing__currency, text.end-price').show();
        lpc.plot();
    } else if (action === 'show') {
        jQuery('rect.end-price').attr('width', '50px').attr('x', basicX);
        jQuery('path.end-price-triangle, text.lp_dynamicPricing__currency, text.end-price').hide();
        jQuery('.lp_dynamicPricing__end-price-input').attr('x', basicX + 2).show();
        jQuery('.lp_dynamicPricing__end-price-input input').val(plotPrice.toFixed(2));
    } else if (action === 'update') {
        if (jQuery('.lp_dynamicPricing__end-price-input input').is(':visible')) {
            var diff = Math.abs(plotPrice - inputPrice);
            if (diff > dragShake) {
                jQuery('.lp_dynamicPricing__end-price-input input').val(plotPrice.toFixed(2));
            }
